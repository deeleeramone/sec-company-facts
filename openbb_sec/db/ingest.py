"""DuckDB ingest for the SEC store."""
...
from openbb_sec.db.backend import DuckDBConn, connect_dolt, table_name as _backend_table, using_dolt
from openbb_sec.db.schema import DATABASE_NAME, init_db
from openbb_sec.utils.company_facts import MULTI_CIK_TICKERS, resolve_company_facts
from openbb_sec.utils.definitions import HEADERS, SEC_DB_PATH
...
def _connect(db_path: str | None = None):
    """Return a backend-appropriate connection wrapper.

    For ``SEC_DB_BACKEND=dolt`` the ``db_path`` argument is ignored; connection
    details come from the ``DOLT_SQL_*`` environment variables and the schema is
    assumed already present (created by ``dolt clone`` of the upstream repo).
    """
    if using_dolt():
        conn = connect_dolt()
    ...
    target = Path(db_path or SEC_DB_PATH)
    target.parent.mkdir(parents=True, exist_ok=True)
    connected_path = str(target)
    print(f"[db] connect open path={target}", flush=True)
    try:
        raw = duckdb.connect(database=str(target))
    except Exception as err:
        print(f"[db] connect failed path={target} error={err!r}", flush=True)
        if target.exists() or target.is_symlink():
            try:
                target.unlink()
                print(f"[db] removed invalid db path={target}", flush=True)
            except Exception as unlink_err:
                print(f"[db] failed removing invalid db path={target} error={unlink_err!r}", flush=True)
                raise
        print(f"[db] creating fresh db at path={target}", flush=True)
        raw = duckdb.connect(database=str(target))

    raw.execute("PRAGMA memory_limit='1536MB'")
    raw.execute("PRAGMA threads=2")
    init_db(raw)
    conn = DuckDBConn(raw)
    _OPEN_CONNECTIONS.add(conn)
    print(f"[db] connect ready path={connected_path}", flush=True)
    return conn


# Connections opened by _connect that have not yet been finalized. DuckDB only
# folds the -wal file back into the main database file on a clean close (or an
# explicit CHECKPOINT), so a leaked/abandoned connection leaves a stranded WAL
# and a database that looks half-written. We track every connection and make
# sure it gets checkpointed + closed, both at the end of each write entry point
# and as a process-exit safety net for crashes.
_OPEN_CONNECTIONS: set[Any] = set()


def _finalize(conn) -> None:
    """Checkpoint (DuckDB) and close the connection.

    Safe to call more than once and safe to call on an already-closed
    connection — all failures are swallowed so finalize never masks the real
    error on an exception path.
    """
...
def ingest_exchange_rates(db_path: str | None = None, lookback_days: int = 3650) -> dict[str, int]:
    ...
    Idempotent: clears the table then reloads, so repeated runs (full ingest and
    cron --update) don't accumulate duplicate rows. ECB publishes ~30 major
    currencies; statements in any currency ECB does not cover stay unconverted
    and are excluded from USD aggregations by the read-side queries.
    """
    from openbb_sec.utils.exchange_rates import load_exchange_rates  # pylint: disable=import-outside-toplevel

    print("[rates] ingest start", flush=True)
    conn = _connect(db_path)
    try:
        conn.execute(f"DELETE FROM {_table('exchange_rates')}")
