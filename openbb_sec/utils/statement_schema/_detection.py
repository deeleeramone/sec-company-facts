        ...
        for tag_data in ns_facts.values():
            for entries_list in tag_data.get("units", {}).values():
                for entry in entries_list:
                    ...
                    # A filing cannot report a period that has not yet ended;
                    # entries with filed < end are forward-looking projections
                    # (e.g. ``FutureAmortizationExpenseYearFive``,
                    # ``LongTermDebtMaturitiesRepaymentsOfPrincipalInYearFive``)
                    # whose ``fy`` belongs to the projecting filing, not to
                    # the projected period.  Without this filter the
                    # earliest-filed-wins tie-breaker below picks the
                    # projection's ancient ``fy`` over the actual report.
                    if filed < end:
                        continue
