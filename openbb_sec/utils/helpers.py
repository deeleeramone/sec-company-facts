...
def _zpad_cik(cik: str | int) -> str:
    """Normalise a CIK to a 10-digit zero-padded string."""
    text = str(cik).strip()
...
def _chdb_rows(sql: str) -> list[tuple]:
    """Run a SELECT against the SEC DuckDB file and return list-of-tuples.

    Function name is kept for back-compat.
    """
    import duckdb  # pylint: disable=import-outside-toplevel

    client = duckdb.connect(database=SEC_DB_PATH, read_only=True)
    try:
        return [tuple(r) for r in client.execute(sql).fetchall()]
    finally:
        client.close()
...
async def get_all_companies(use_cache: bool = True) -> DataFrame:
    ...
    rows = _chdb_rows(
        f"SELECT cik, ticker AS symbol, name "
        f"FROM {DATABASE_NAME}.tickers ORDER BY ticker"
    )
    return DataFrame(rows, columns=["cik", "symbol", "name"]).astype(str)


async def get_all_ciks(use_cache: bool = True) -> DataFrame:
    """Get a list of entity names and their CIK number."""
    rows = _chdb_rows(
        f"SELECT entity_name, cik FROM {DATABASE_NAME}.entities "
        f"ORDER BY entity_name"
    )
    return DataFrame(rows, columns=["Institution", "CIK Number"]).astype(str)


async def get_mf_and_etf_map(use_cache: bool = True) -> DataFrame:
    """Return the CIK / seriesId / classId / symbol map for funds and ETFs."""
    rows = _chdb_rows(
        f"SELECT cik, series_id AS seriesId, class_id AS classId, symbol "
        f"FROM {DATABASE_NAME}.funds"
    )
    return DataFrame(rows, columns=["cik", "seriesId", "classId", "symbol"]).astype(str)
...
async def search_institutions(keyword: str, use_cache: bool = True) -> DataFrame:
    ...
    rows = _chdb_rows(
        f"SELECT entity_name, cik FROM {DATABASE_NAME}.entities "
        f"WHERE entity_name ILIKE {_q(pat)} ORDER BY entity_name"
    )
    return DataFrame(rows, columns=["Institution", "CIK Number"]).astype(str)
...
async def symbol_map(symbol: str, use_cache: bool = True) -> str:
    ...
    Resolution order: ``multi_cik_tickers`` (newest/primary CIK) →
    ``tickers`` (common stocks) → ``funds`` (mutual funds / ETFs).
    """
    sym = symbol.upper().replace(".", "-")
    multi = _chdb_rows(
        f"SELECT cik FROM {DATABASE_NAME}.multi_cik_tickers "
        f"WHERE ticker = {_q(sym)} ORDER BY priority LIMIT 1"
    )
    if multi:
        return multi[0][0]
    common = _chdb_rows(
        f"SELECT cik FROM {DATABASE_NAME}.tickers "
        f"WHERE ticker = {_q(sym)} LIMIT 1"
    )
    if common:
        return common[0][0]
    fund = _chdb_rows(
        f"SELECT cik FROM {DATABASE_NAME}.funds "
        f"WHERE symbol = {_q(sym)} LIMIT 1"
    )
    return fund[0][0] if fund else ""


async def cik_map(cik: str | int, use_cache: bool = True) -> str:
    """Convert a CIK number to a ticker symbol (common stocks only)."""
    cik_padded = _zpad_cik(cik)
    rows = _chdb_rows(
        f"SELECT ticker FROM {DATABASE_NAME}.tickers "
        f"WHERE cik = {_q(cik_padded)} ORDER BY ticker LIMIT 1"
    )
    if not rows:
        return f"Error: CIK, {cik_padded}, does not have a unique ticker."
    return rows[0][0]
